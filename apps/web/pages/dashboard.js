  ...
  return (
    <div style={{ maxWidth: 1000, margin: "40px auto", padding: 20 }}>
      ...
      <section style={{ marginTop: 24 }}>
        <h2>Piyasa Durumu</h2>
        <div style={{padding:20,background:"#121824",border:"1px solid #1F2937",borderRadius:12}}>
      ...
      <section style={{ marginTop: 24, display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14 }}>
        <div style={{background:"#121824",padding:16,borderRadius:12}}>
          <div>Çalışan Robot</div><div style={{fontSize:24,fontWeight:700}}>{kpis.robots}</div>
        </div>
        <div style={{background:"#121824",padding:16,borderRadius:12}}>
          <div>Toplam PnL</div><div style={{fontSize:24,fontWeight:700,color:"var(--green)"}}>₺{kpis.pnl}</div>
        </div>
        <div style={{background:"#121824",padding:16,borderRadius:12}}>
          <div>Aktif Kullanıcı</div><div style={{fontSize:24,fontWeight:700}}>{kpis.users}</div>
        </div>
      </section>
      <section style={{ marginTop: 24 }}>
        <h2>📢 Haber Akışı</h2>
        <ul style={{listStyle:"none",padding:0}}>
          {news.map(n=>(<li key={n.id} style={{margin:"12px 0",padding:12,background:"#121824",borderRadius:8}}>
